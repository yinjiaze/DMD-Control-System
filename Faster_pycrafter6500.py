...
## header creation
    ...
    width=convlen(1920,16) ##convert int 1920 to a bit string with 16 length-> 0000011110000000
    width=bitstobytes(width) ## convert 16 length to a byte  -> [128, 7]
...
def generate_encoded(matrix):

    arr = []
    arr.append(matrix)
    print('merging...')
    imagedata = mergeimages(arr[0:])
    print('encoding...')
    imagedata, size = encode(imagedata)
    encoded_data = [imagedata, size]
...
## size of package description bytes=2. 64-4-2-2=56
    ...
    def defsequence(self,encoded_data,exp,ti,dt,to,rep):

        self.stopsequence()
        encodedimages = []
        sizes = []

        imagedata, size = encoded_data
        encodedimages.append(imagedata)
        sizes.append(size)


        self.definepattern(0,exp[0],1,'111',ti[0],dt[0],to[0],0,0)

        self.configurelut(1,rep)


        self.setbmp(0,sizes[0])


        print ('uploading...')
        self.bmpload(encodedimages[0], sizes[0])
